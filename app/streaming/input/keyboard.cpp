...
void SdlInputHandler::performSpecialKeyCombo(KeyCombo combo)
{
    ...
    case KeyComboToggleVsrOverlay:
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "Detected VSR settings toggle combo");

        {
            Overlay::OverlayManager& overlayMgr = Session::get()->getOverlayManager();
            bool newState = !overlayMgr.isOverlayEnabled(Overlay::OverlayVsrSettings);
            ...
            overlayMgr.setOverlayState(Overlay::OverlayVsrSettings, newState);
        }
        break;
...
void SdlInputHandler::updateVsrOverlayText()
{
    Overlay::OverlayManager& overlayMgr = Session::get()->getOverlayManager();
    StreamingPreferences* prefs = Session::get()->m_Preferences;
    ...
    if (m_VsrOverlaySelectedSection == 0) {
        snprintf(contentLines, sizeof(contentLines),
            "%s Detail:   [%s]  Strength: %4d\n"
            "%s Denoise:  [%s]  Strength: %4d\n"
            "%s Deband:   [%s]  Strength: %4d",
            m_VsrOverlaySelectedRow == 0 ? ">" : " ",
            prefs->detailEnabled ? "ON " : "OFF", prefs->detailStrength,
            m_VsrOverlaySelectedRow == 1 ? ">" : " ",
            prefs->denoiseEnabled ? "ON " : "OFF", prefs->denoiseStrength,
            m_VsrOverlaySelectedRow == 2 ? ">" : " ",
            prefs->debandEnabled ? "ON " : "OFF", prefs->debandStrength);
    } else if (m_VsrOverlaySelectedSection == 1) {
        snprintf(contentLines, sizeof(contentLines),
            "%s Absolute Mouse:  [%s]\n"
            "%s Swap Buttons:    [%s]\n"
            "%s Reverse Scroll:  [%s]",
            m_VsrOverlaySelectedRow == 0 ? ">" : " ",
            prefs->absoluteMouseMode ? "ON " : "OFF",
            m_VsrOverlaySelectedRow == 1 ? ">" : " ",
            prefs->swapMouseButtons ? "ON " : "OFF",
            m_VsrOverlaySelectedRow == 2 ? ">" : " ",
            prefs->reverseScrollDirection ? "ON " : "OFF");
        ...
        snprintf(contentLines, sizeof(contentLines),
            ...
            "  HDR:           [%s]   VRR: [%s]",
            m_VsrOverlaySelectedRow == 0 ? ">" : " ",
            prefs->framePacing ? "ON " : "OFF",
            m_VsrOverlaySelectedRow == 1 ? ">" : " ",
            prefs->useDisplayLink ? "ON " : "OFF",
            m_VsrOverlaySelectedRow == 2 ? ">" : " ",
    ...
    snprintf(textBuf, overlayMgr.getOverlayMaxTextLength(),
        ...
        sectionTabs, contentLines);
    
    overlayMgr.setOverlayTextUpdated(Overlay::OverlayVsrSettings);
}
...
void SdlInputHandler::handleKeyEvent(SDL_KeyboardEvent* event)
{
    ...
    // Handle VSR overlay navigation when it's visible
    if (Session::get()->getOverlayManager().isOverlayEnabled(Overlay::OverlayVsrSettings) &&
        event->state == SDL_PRESSED) {
        StreamingPreferences* prefs = Session::get()->m_Preferences;
        bool handled = true;
        int step = (event->keysym.mod & KMOD_SHIFT) ? 25 : 5;

        // Only consume overlay navigation keys without Ctrl/Alt/GUI modifiers,
        // except Escape which should always close the overlay.
        const SDL_Keycode overlayKey = event->keysym.sym;
        const bool isOverlayNavKey = (overlayKey == SDLK_TAB || overlayKey == SDLK_UP ||
                                      overlayKey == SDLK_DOWN || overlayKey == SDLK_LEFT ||
                                      overlayKey == SDLK_RIGHT || overlayKey == SDLK_RETURN ||
                                      overlayKey == SDLK_KP_ENTER || overlayKey == SDLK_ESCAPE);
        if (isOverlayNavKey && overlayKey != SDLK_ESCAPE) {
            const SDL_Keymod blockedMods = (SDL_Keymod)(KMOD_CTRL | KMOD_ALT | KMOD_GUI);
            if ((event->keysym.mod & blockedMods) != 0) {
                handled = false;
            }
        }

        if (!handled) {
            // Allow regular shortcut and host input processing.
        }
        else {
        ...
        case SDLK_TAB:
            ...
            break;
        case SDLK_UP:
            m_VsrOverlaySelectedRow = (m_VsrOverlaySelectedRow + currentSectionRowCount - 1) % currentSectionRowCount;
            break;
        case SDLK_DOWN:
            m_VsrOverlaySelectedRow = (m_VsrOverlaySelectedRow + 1) % currentSectionRowCount;
            break;
        case SDLK_LEFT:
            if (m_VsrOverlaySelectedSection == 0) {
                if (m_VsrOverlaySelectedRow == 0) {
                    prefs->detailStrength = qMax(-100, prefs->detailStrength - step);
                } else if (m_VsrOverlaySelectedRow == 1) {
                    prefs->denoiseStrength = qMax(0, prefs->denoiseStrength - step);
                } else {
                    prefs->debandStrength = qMax(0, prefs->debandStrength - step);
                }
            } else if (m_VsrOverlaySelectedSection == 1) {
                if (m_VsrOverlaySelectedRow == 0) {
                    prefs->absoluteMouseMode = !prefs->absoluteMouseMode;
                } else if (m_VsrOverlaySelectedRow == 1) {
                    prefs->swapMouseButtons = !prefs->swapMouseButtons;
                } else {
                    prefs->reverseScrollDirection = !prefs->reverseScrollDirection;
                }
            } else if (m_VsrOverlaySelectedSection == 2) {
                ...
                if (m_VsrOverlaySelectedRow == 0) {
                    ...
                    prefs->useDisplayLink = !prefs->useDisplayLink;
                } else {
        ...
        case SDLK_RIGHT:
            if (m_VsrOverlaySelectedSection == 0) {
                if (m_VsrOverlaySelectedRow == 0) {
                    prefs->detailStrength = qMin(100, prefs->detailStrength + step);
                } else if (m_VsrOverlaySelectedRow == 1) {
                    prefs->denoiseStrength = qMin(100, prefs->denoiseStrength + step);
                } else {
                    prefs->debandStrength = qMin(100, prefs->debandStrength + step);
                }
            } else if (m_VsrOverlaySelectedSection == 1) {
                if (m_VsrOverlaySelectedRow == 0) {
                    prefs->absoluteMouseMode = !prefs->absoluteMouseMode;
        ...
        case SDLK_KP_ENTER:
            if (m_VsrOverlaySelectedSection == 0) {
                if (m_VsrOverlaySelectedRow == 0) {
                    prefs->detailEnabled = !prefs->detailEnabled;
                } else if (m_VsrOverlaySelectedRow == 1) {
                    prefs->denoiseEnabled = !prefs->denoiseEnabled;
                } else {
                    prefs->debandEnabled = !prefs->debandEnabled;
                }
            } else if (m_VsrOverlaySelectedSection == 1) {
                if (m_VsrOverlaySelectedRow == 0) {
                    prefs->absoluteMouseMode = !prefs->absoluteMouseMode;
        ...
        case SDLK_ESCAPE:
            prefs->save();
            Session::get()->getOverlayManager().setOverlayState(Overlay::OverlayVsrSettings, false);
            return;
        default:
            handled = false;
            break;
