...
typedef struct _DECODER_PARAMETERS {
    ...
    int vsrColorMode;  // 0=Linear, 1=Perceptual, 2=HDR
    bool detailEnabled;      // Enable detail pass (dehalo/sharpen)
    int detailStrength;      // -100 to +100 (negative=soften, positive=sharpen)
    bool denoiseEnabled;     // Enable denoise pass
    int denoiseStrength;     // 0 to 100
    bool debandEnabled;      // Enable deband pass
    int debandStrength;      // 0 to 100
    int superResolutionMode; // StreamingPreferences::SuperResolutionMode
    bool useDisplayLink;     // Snapshot of display link preference at session start
    bool tripleBuffering;    // Snapshot of triple buffering preference at session start
    bool testOnly;
