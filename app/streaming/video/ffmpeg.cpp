...
// IDR requests are expensive recovery operations. During burst loss, requesting
// too frequently can amplify congestion and jitter. Apply a short cooldown to
// avoid back-to-back requests while still recovering quickly.
#define IDR_REQUEST_COOLDOWN_US 250000

#ifdef Q_OS_DARWIN
static MoonlightStatTracker s_DecodeQueueDepthStats;
static MoonlightStatTracker s_ReceiveFrameStats;
static MoonlightStatTracker s_SendPacketStats;
#endif
...
FFmpegVideoDecoder::FFmpegVideoDecoder(bool testOnly)
    : m_Pkt(av_packet_alloc()),
      ...
      m_LastIdrRequestUs(0),
      m_SuppressedIdrRequests(0)
{
...
void FFmpegVideoDecoder::requestIdrFrameWithCooldown(const char* reason)
{
    const uint64_t nowUs = LiGetMicroseconds();
    if (m_LastIdrRequestUs != 0 && nowUs - m_LastIdrRequestUs < IDR_REQUEST_COOLDOWN_US) {
        m_SuppressedIdrRequests++;
#ifdef Q_OS_DARWIN
        if (m_SuppressedIdrRequests == 1 || (m_SuppressedIdrRequests % 10) == 0) {
            ML_LOG_VIDEO_WARN("IDR request suppressed (%u total, reason=%s)",
                              m_SuppressedIdrRequests,
                              reason);
        }
#endif
    ...
    m_LastIdrRequestUs = nowUs;
#ifdef Q_OS_DARWIN
    if (m_SuppressedIdrRequests != 0) {
        ML_LOG_VIDEO("IDR request resumed after suppressing %u request(s)",
                     m_SuppressedIdrRequests);
        m_SuppressedIdrRequests = 0;
    }
#endif
...
{
#ifdef Q_OS_DARWIN
    ml_stat_init(&s_DecodeQueueDepthStats);
    ml_stat_init(&s_ReceiveFrameStats);
    ml_stat_init(&s_SendPacketStats);
#endif
    ...
    if (testMode != TestMode::TestFrameOnly) {
        ...
        m_Pacer = new Pacer(m_FrontendRenderer, &m_ActiveWndVideoStats, prefs->framePacingMode);
...
{
#ifdef Q_OS_DARWIN
    // Use USER_INITIATED to prioritize decode without starving peer threads.
    if (pthread_set_qos_class_self_np(QOS_CLASS_USER_INITIATED, 0) == 0) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "Decoder thread QoS set to USER_INITIATED");
    }
#endif
    ...
    while (!SDL_AtomicGet(&m_DecoderThreadShouldQuit)) {
        ...
        if (m_FramesIn != m_FramesOut) {
            ...
            int err;
            do {
                uint64_t receiveStartUs = LiGetMicroseconds();
                err = avcodec_receive_frame(m_VideoDecoderCtx, frame);
#ifdef Q_OS_DARWIN
                ml_stat_add(&s_ReceiveFrameStats, (double)(LiGetMicroseconds() - receiveStartUs));
#endif
                if (err == 0) {
...
#ifdef Q_OS_DARWIN
                    int decodeQueueDepth = m_FramesIn - m_FramesOut;
                    if (decodeQueueDepth < 0) {
                        decodeQueueDepth = 0;
                    }
                    ml_stat_add(&s_DecodeQueueDepthStats, (double)decodeQueueDepth);

                    m_Pacer->notifyDecoderBacklog(decodeQueueDepth);

                    if (ml_stat_should_log(&s_DecodeQueueDepthStats, 5000)) {
                        ML_LOG_VIDEO("Decode stats: queue_avg=%.2f, queue_min=%.0f, queue_max=%.0f, recv_avg=%.2fus, send_avg=%.2fus",
                                     ml_stat_avg(&s_DecodeQueueDepthStats),
                                     s_DecodeQueueDepthStats.min,
                                     s_DecodeQueueDepthStats.max,
                                     ml_stat_avg(&s_ReceiveFrameStats),
                                     ml_stat_avg(&s_SendPacketStats));
                        ml_stat_reset(&s_DecodeQueueDepthStats);
                        ml_stat_reset(&s_ReceiveFrameStats);
                        ml_stat_reset(&s_SendPacketStats);
                    }
#endif
                    ...
                    requestIdrFrameWithCooldown("avcodec_receive_frame_failed");
                }
...
int FFmpegVideoDecoder::submitDecodeUnit(PDECODE_UNIT du)
{
    ...
    if (m_FramesIn == 0 && du->frameType != FRAME_TYPE_IDR) {
        requestIdrFrameWithCooldown("first_frame_not_idr");
        return DR_NEED_IDR;
    }
    ...
    uint64_t sendStartUs = LiGetMicroseconds();
    err = avcodec_send_packet(m_VideoDecoderCtx, m_Pkt);
#ifdef Q_OS_DARWIN
