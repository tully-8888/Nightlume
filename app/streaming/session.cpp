...
#ifdef Q_OS_DARWIN
    ML_LOG_SESSION_ERROR("Connection terminated with error code: %d (0x%08X)", errorCode, errorCode);
#endif
                            ...
                            int frameRate, bool enableVsync, bool enableFramePacing, bool enableVideoEnhancement, bool testOnly, IVideoDecoder*& chosenDecoder)
{
    ...
    params.vsrColorMode = static_cast<int>(StreamingPreferences::get()->vsrColorMode);
    params.detailEnabled = StreamingPreferences::get()->detailEnabled;
    params.detailStrength = StreamingPreferences::get()->detailStrength;
    params.denoiseEnabled = StreamingPreferences::get()->denoiseEnabled;
    params.denoiseStrength = StreamingPreferences::get()->denoiseStrength;
    params.debandEnabled = StreamingPreferences::get()->debandEnabled;
    params.debandStrength = StreamingPreferences::get()->debandStrength;
    params.superResolutionMode = static_cast<int>(StreamingPreferences::get()->superResolutionMode);
    params.useDisplayLink = StreamingPreferences::get()->useDisplayLink;
    params.tripleBuffering = StreamingPreferences::get()->tripleBuffering;
...
bool Session::startConnectionAsync()
{
#ifdef Q_OS_DARWIN
    ml_debug_log_init();
    ML_LOG_SESSION("Starting async connection to %s (app: %s)",
                   m_Computer->name.toUtf8().constData(),
                   m_App.name.toUtf8().constData());
    ML_LOG_SESSION("Stream config: %dx%d @ %d fps, bitrate: %d kbps",
                   m_StreamConfig.width, m_StreamConfig.height,
                   m_StreamConfig.fps, m_StreamConfig.bitrate);
    ML_LOG_NETWORK("Preferences: packetSize=%d, framePacing=%s, framePacingMode=%d, videoEnhancing=%s, codecConfig=%d, decoderSelection=%d, HDR=%s",
                   m_Preferences->packetSize,
                   m_Preferences->framePacing ? "on" : "off",
                   (int)m_Preferences->framePacingMode,
                   m_Preferences->videoEnhancing ? "on" : "off",
                   (int)m_Preferences->videoCodecConfig,
                   (int)m_Preferences->videoDecoderSelection,
                   m_Preferences->enableHdr ? "on" : "off");
    ML_LOG_SESSION("Feature snapshot: gameOpt=%s, autoBitrate=%s, unlockBitrate=%s, keepAwake=%s, AWDL=%s, displayLink=%s, tripleBuffer=%s",
                   m_Preferences->gameOptimizations ? "on" : "off",
                   m_Preferences->autoAdjustBitrate ? "on" : "off",
                   m_Preferences->unlockBitrate ? "on" : "off",
                   m_Preferences->keepAwake ? "on" : "off",
                   m_Preferences->suppressAWDL ? "on" : "off",
                   m_Preferences->useDisplayLink ? "on" : "off",
                   m_Preferences->tripleBuffering ? "on" : "off");
    ML_LOG_METALFX("VSR snapshot: mode=%d, color=%d, detail=%s(%d), denoise=%s(%d), deband=%s(%d)",
                   (int)m_Preferences->superResolutionMode,
                   (int)m_Preferences->vsrColorMode,
                   m_Preferences->detailEnabled ? "on" : "off",
                   m_Preferences->detailStrength,
                   m_Preferences->denoiseEnabled ? "on" : "off",
                   m_Preferences->denoiseStrength,
                   m_Preferences->debandEnabled ? "on" : "off",
                   m_Preferences->debandStrength);
#endif
    ...
    if (m_Preferences->packetSize != 0) {
        ...
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                    ...
                    m_Preferences->packetSize);
#ifdef Q_OS_DARWIN
        ML_LOG_NETWORK_WARN("Using custom packet size override: %d bytes (forcing STREAM_CFG_LOCAL)",
                            m_Preferences->packetSize);
#endif
    }
    else {
        ...
        case NvComputer::RI_LAN:
            ...
            m_StreamConfig.streamingRemotely = STREAM_CFG_LOCAL;
#ifdef Q_OS_DARWIN
            ML_LOG_NETWORK("Reachability=LAN, packet size=%d, streamingRemotely=LOCAL", m_StreamConfig.packetSize);
#endif
            break;
...
#ifdef Q_OS_DARWIN
            ML_LOG_NETWORK_WARN("Reachability=VPN, capping packet size to %d, streamingRemotely=REMOTE",
...
#ifdef Q_OS_DARWIN
            ML_LOG_NETWORK("Reachability=UNKNOWN, packet size=%d, streamingRemotely=AUTO",
                           m_StreamConfig.packetSize);
#endif
            break;
        }
    ...
    if (err != 0) {
#ifdef Q_OS_DARWIN
        ML_LOG_SESSION_ERROR("LiStartConnection failed: err=%d, packetSize=%d, streamingRemotely=%d, videoFormatMask=0x%X",
                             err,
                             m_StreamConfig.packetSize,
                             m_StreamConfig.streamingRemotely,
...
#ifdef Q_OS_DARWIN
    ML_LOG_SESSION("Connection started: packetSize=%d, streamingRemotely=%d, selectedVideoMask=0x%X",
                   m_StreamConfig.packetSize,
                   m_StreamConfig.streamingRemotely,
                   m_StreamConfig.supportedVideoFormats);
#endif
...
#ifdef Q_OS_DARWIN
        CFStringRef reason = CFSTR("Game streaming in progress");
        IOReturn ret = IOPMAssertionCreateWithName(
            kIOPMAssertionTypePreventUserIdleSystemSleep,
            kIOPMAssertionLevelOn,
            reason,
            (IOPMAssertionID*)&m_PowerAssertionId);
        if (ret == kIOReturnSuccess) {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                        "Power assertion created to prevent idle sleep");
        }

        IOReturn displayRet = IOPMAssertionCreateWithName(
            kIOPMAssertionTypePreventUserIdleDisplaySleep,
            kIOPMAssertionLevelOn,
            CFSTR("Game streaming in progress (display)"),
            (IOPMAssertionID*)&m_DisplayAssertionId);
        if (displayRet == kIOReturnSuccess) {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                        "Display assertion created to prevent display sleep");
        }
#endif
    }

#ifdef Q_OS_DARWIN
    if (!m_AppNapActivityToken) {
        m_AppNapActivityToken = MoonlightBeginLatencyCriticalActivity("Game streaming session");
    }
    if (m_Preferences->suppressAWDL) {
        MoonlightStartAWDLSuppressionTimer();
    }
#endif
...
#ifdef Q_OS_DARWIN
    if (m_PowerAssertionId != 0) {
        IOPMAssertionRelease((IOPMAssertionID)m_PowerAssertionId);
        m_PowerAssertionId = 0;
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "Power assertion released");
    }
    if (m_DisplayAssertionId != 0) {
        IOPMAssertionRelease((IOPMAssertionID)m_DisplayAssertionId);
        m_DisplayAssertionId = 0;
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "Display assertion released");
    }
    if (m_AppNapActivityToken) {
        MoonlightEndLatencyCriticalActivity(m_AppNapActivityToken);
        m_AppNapActivityToken = nullptr;
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "App Nap activity token released");
    }
    MoonlightStopAWDLSuppressionTimer();
#endif
