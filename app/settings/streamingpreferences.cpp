...
#define SER_VSRCOLORMODE "vsrcolormode"
#define SER_DETAILENABLED "detailenabled"
#define SER_DETAILSTRENGTH "detailstrength"
#define SER_DENOISEENABLED "denoiseenabled"
#define SER_DENOISESTRENGTH "denoisestrength"
#define SER_DEBANDENABLED "debandenabled"
#define SER_DEBANDSTRENGTH "debandstrength"
...
void StreamingPreferences::reload()
{
    ...
    framePacingMode = static_cast<FramePacingMode>(settings.value(SER_FRAMEPACINGMODE,
                                                   static_cast<int>(FramePacingMode::FPM_BALANCED)).toInt());
...
#endif
    vsrColorMode = static_cast<VsrColorMode>(settings.value(SER_VSRCOLORMODE,
                                                            static_cast<int>(VsrColorMode::VCM_PERCEPTUAL)).toInt());
    detailEnabled = settings.value(SER_DETAILENABLED, false).toBool();
    detailStrength = settings.value(SER_DETAILSTRENGTH, 0).toInt();
    denoiseEnabled = settings.value(SER_DENOISEENABLED, false).toBool();
    denoiseStrength = settings.value(SER_DENOISESTRENGTH, 50).toInt();
    debandEnabled = settings.value(SER_DEBANDENABLED, false).toBool();
    debandStrength = settings.value(SER_DEBANDSTRENGTH, 50).toInt();
...
void StreamingPreferences::save()
{
    ...
    settings.setValue(SER_VSRCOLORMODE, static_cast<int>(vsrColorMode));
    settings.setValue(SER_DETAILENABLED, detailEnabled);
    settings.setValue(SER_DETAILSTRENGTH, detailStrength);
    settings.setValue(SER_DENOISEENABLED, denoiseEnabled);
    settings.setValue(SER_DENOISESTRENGTH, denoiseStrength);
    settings.setValue(SER_DEBANDENABLED, debandEnabled);
    settings.setValue(SER_DEBANDSTRENGTH, debandStrength);
