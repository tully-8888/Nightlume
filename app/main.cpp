...
#ifdef Q_OS_DARWIN
#include "streaming/macos/macos_performance.h"
#endif

#if !defined(QT_DEBUG) && defined(Q_OS_DARWIN)
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--perf-selftest") == 0) {
            int holdSeconds = 5;
            size_t mlockBytes = 4 * 1024 * 1024;
            for (int j = i + 1; j < argc; j++) {
                if (strcmp(argv[j], "--hold-seconds") == 0 && j + 1 < argc) {
                    holdSeconds = atoi(argv[j + 1]);
                } else if (strcmp(argv[j], "--mlock-bytes") == 0 && j + 1 < argc) {
                    mlockBytes = (size_t)atoll(argv[j + 1]);
                }
            }
            return MoonlightPerfSelftest(holdSeconds, mlockBytes);
        }
        if (strcmp(argv[i], "--install-helper") == 0 || strcmp(argv[i], "--reinstall-helper") == 0) {
            int force = (strcmp(argv[i], "--reinstall-helper") == 0) ? 1 : 0;
            int result = MoonlightInstallHelper(force);
            if (result == 0) {
                printf("Helper installed successfully.\n");
            } else if (result == 1) {
                printf("Helper already installed.\n");
            } else {
                printf("Helper installation failed (code %d).\n", result);
            }
            return result < 0 ? 1 : 0;
        }
    }
