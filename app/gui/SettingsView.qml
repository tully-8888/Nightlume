    ...
    Column {
        ...
        GroupBox {
            ...
            Column {
                ...
                AutoResizingComboBox {
                    id: framePacingModeComboBox
                    enabled: StreamingPreferences.enableVsync && StreamingPreferences.framePacing
                    textRole: "text"
                    model: ListModel {
                        id: framePacingModeListModel
                        ListElement {
                            text: qsTr("Balanced (3 frames)")
                            val: 0
                        }
                        ListElement {
                            text: qsTr("Low Latency (2 frames)")
                            val: 1
                        }
                        ListElement {
                            text: qsTr("Ultra Low (1 frame)")
                            val: 2
                        }
                    }
                    function reinitialize() {
                        currentIndex = StreamingPreferences.framePacingMode
                    }
                    Component.onCompleted: reinitialize()
                    Connections {
                        target: StreamingPreferences
                        function onFramePacingModeChanged() {
                            framePacingModeComboBox.reinitialize()
                        }
                    }
                    onActivated: {
                        StreamingPreferences.framePacingMode = framePacingModeListModel.get(currentIndex).val
                    }

                    ToolTip.delay: 1000
                    ToolTip.timeout: 5000
                    ToolTip.visible: hovered
                    ToolTip.text: qsTr("Controls latency vs stability tradeoff. Lower = less latency but more stutter risk.")
                }
                ...
                Label {
                    visible: videoEnhancementCheck.checked
                    width: parent.width
                    id: vsrColorModeTitle
                    text: qsTr("Color Processing Mode")
                    font.pointSize: 12
                    wrapMode: Text.Wrap
                }
                ...
                AutoResizingComboBox {
                    ...
                    ToolTip.timeout: 5000
                    ToolTip.visible: hovered
                    ToolTip.text: qsTr("Controls how MetalFX processes colors during upscaling. Perceptual is recommended for video streaming. Linear is designed for native game rendering. HDR is for high dynamic range content.")

                    Component.onCompleted: {
                        var saved_color_mode = StreamingPreferences.vsrColorMode
                        currentIndex = 0
                        for (var i = 0; i < vsrColorModeListModel.count; i++) {
                            var el_color_mode = vsrColorModeListModel.get(i).val;
                            if (saved_color_mode === el_color_mode) {
                                currentIndex = i
                                break
                            }
                        }
                    ...
                    id: vsrColorModeComboBox
                    textRole: "text"
                    model: ListModel {
                        id: vsrColorModeListModel

                        ListElement {
                            text: qsTr("Perceptual (Recommended)")
                            val: StreamingPreferences.VCM_PERCEPTUAL
                        }
                        ListElement {
                            text: qsTr("Linear")
                            val: StreamingPreferences.VCM_LINEAR
                        }
                        ListElement {
                            text: qsTr("HDR")
                            val: StreamingPreferences.VCM_HDR
                        }
                    }
                    onActivated: {
                        StreamingPreferences.vsrColorMode = vsrColorModeListModel.get(currentIndex).val
                    }
                }

                CheckBox {
                    id: detailCheck
                    text: qsTr("Detail Adjustment")
                    font.pointSize: 12
                    checked: StreamingPreferences.detailEnabled
                    onCheckedChanged: {
                        StreamingPreferences.detailEnabled = checked
                    }
                    visible: StreamingPreferences.videoEnhancing
                    
                    ToolTip.delay: 1000
                    ToolTip.timeout: 5000
                    ToolTip.visible: hovered
                    ToolTip.text: qsTr("Adjust image sharpness. Negative values reduce haloing/ringing artifacts. Positive values sharpen.")
                }

                Label {
                    text: qsTr("Strength: %1").arg(detailSlider.value)
                    font.pointSize: 10
                    visible: StreamingPreferences.videoEnhancing && StreamingPreferences.detailEnabled
                }
                ...
                Slider {
                    ...
                    to: 100
                    stepSize: 5
                    value: StreamingPreferences.detailStrength
                    onValueChanged: {
                        StreamingPreferences.detailStrength = value
                    }
                    visible: StreamingPreferences.videoEnhancing && StreamingPreferences.detailEnabled
                ...
                CheckBox {
                    ...
                    text: qsTr("Denoise")
                    font.pointSize: 12
                    checked: StreamingPreferences.denoiseEnabled
                    onCheckedChanged: {
                        StreamingPreferences.denoiseEnabled = checked
                    }
                    visible: StreamingPreferences.videoEnhancing
                    ...
                    ToolTip.timeout: 5000
                    ToolTip.visible: hovered
                    ToolTip.text: qsTr("Reduce compression artifacts and noise in the video stream.")
                }

                Label {
                    text: qsTr("Strength: %1").arg(denoiseSlider.value)
                    font.pointSize: 10
                    visible: StreamingPreferences.videoEnhancing && StreamingPreferences.denoiseEnabled
                }
                ...
                Slider {
                    ...
                    to: 100
                    stepSize: 5
                    value: StreamingPreferences.denoiseStrength
                    onValueChanged: {
                        StreamingPreferences.denoiseStrength = value
                    }
                    visible: StreamingPreferences.videoEnhancing && StreamingPreferences.denoiseEnabled
                    Layout.fillWidth: true
                }
                ...
                CheckBox {
                    ...
                    text: qsTr("Debanding")
                    font.pointSize: 12
                    checked: StreamingPreferences.debandEnabled
                    onCheckedChanged: {
                        StreamingPreferences.debandEnabled = checked
                    }
                    visible: StreamingPreferences.videoEnhancing
                    ...
                    ToolTip.timeout: 5000
                    ToolTip.visible: hovered
                    ToolTip.text: qsTr("Reduce banding artifacts in gradients, especially useful for HDR content.")
                }

                Label {
                    text: qsTr("Strength: %1").arg(debandSlider.value)
                    font.pointSize: 10
                    visible: StreamingPreferences.videoEnhancing && StreamingPreferences.debandEnabled
                }
                ...
                Slider {
                    ...
                    to: 100
                    stepSize: 5
                    value: StreamingPreferences.debandStrength
                    onValueChanged: {
                        StreamingPreferences.debandStrength = value
                    }
                    visible: StreamingPreferences.videoEnhancing && StreamingPreferences.debandEnabled
                    Layout.fillWidth: true
                }
        ...
        GroupBox {
            ...
            Column {
                ...
                CheckBox {
                    ...
                    text: qsTr("Suppress AirDrop/AirPlay during streaming (macOS)")
                    font.pointSize: 12
                    checked: StreamingPreferences.suppressAWDL
                    onCheckedChanged: {
                        StreamingPreferences.suppressAWDL = checked
                    }

                    ToolTip.delay: 1000
                    ToolTip.timeout: 5000
                    ToolTip.visible: hovered
                    ToolTip.text: qsTr("Disables the AWDL interface (used by AirDrop/AirPlay) during streaming to reduce WiFi interference. Disable this if you experience network issues.")
                }
